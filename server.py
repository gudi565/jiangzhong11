...
def _glm_plagiarism_judge(text):
    """让 GLM 做原创度分析，基于「常见句占比」量化。"""
    prompt = (
        "你读过海量中文学术论文、教科书、百科。判断这段文字中【有多少比例的句子是常见表达】。\n\n"
        "常见表达 = 你在大量论文/教科书中反复见过的标准定义、模板套话、万能背景介绍。\n"
        "原创内容 = 作者独特的观点、个人经历、具体实验数据、特定作品/案例分析。\n\n"
        "评分标准（严格按比例）：\n"
        "90%以上句子是常见表达 → 70-85分\n"
        "60-90%是常见表达 → 45-60分\n"
        "30-60%是常见表达 → 25-40分\n"
        "低于30%是常见表达（大部分原创） → 10-20分\n\n"
        "重要加分项（+10）：完全没有具体的实验、数据、人名、日期、案例、个人经历。\n"
        "重要减分项（-15）：包含个人创作经历、特定作品名、具体实验数据、明确人名日期。\n\n"
        '只输出JSON：{"score":数字,"suspects":["最像常见表达的句子1","句子2"],"reason":"一句评价"}\n\n'
        f"文本：\n{text[:2500]}"
    )
    msg = engine.chat([
